...
public class ReporteDController {
    ...
    @FXML
    private TextField facilitadorField; // 00197822 Campo para ingresar el facilitador de la tarjeta
    ...
    private void initialize() { // 00197822 Método que se ejecuta al inicializar la vista
        btnGenerarReporte.setOnAction(event -> { // 00197822 Acción al presionar el botón de generar reporte
            String facilitador = facilitadorField.getText(); // 00197822 Obtener el texto ingresado en el campo Facilitador

            if (facilitador.isEmpty()) { // 00197822 Validar que el campo Facilitador no esté vacío
                showAlert(Alert.AlertType.ERROR, "Error de Validación", "El campo Facilitador es obligatorio."); // 00197822 Mostrar alerta si el campo está vacío
                return;
            }

            try {
                ReporteService reporteService = new ReporteService(); // 00197822 Crear una instancia de ReporteService
                reporteService.guardarReporteD(facilitador); // 00197822 Generar y guardar el reporte D
                showAlert(Alert.AlertType.INFORMATION, "Éxito", "Reporte generado exitosamente."); // 00197822 Mostrar alerta de éxito
            } catch (Exception e) { // 00197822 Manejar cualquier error
